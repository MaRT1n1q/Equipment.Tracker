...
- ℹ️ В «Настройках» отображаются логин, ID пользователя и адрес Auth API
...
3. **Войдите** по логину и паролю (новый пользователь создастся автоматически)
4. **Готово!** База данных создастся автоматически при первом запуске
...
# 3️⃣ Запуск backend авторизации (обязательно для входа)
# backend находится в соседней папке ../backend
cd ../backend
make run

# 4️⃣ Запуск приложения в режиме разработки
cd ../Equipment.Tracker
...
# 5️⃣ (Опционально) если backend auth API не localhost:9090
# создайте .env и задайте адрес API
echo "VITE_AUTH_API_URL=http://localhost:9090/api/auth/login" > .env
...
| `npm run dev:web`          | Веб-режим (без Electron)             |
